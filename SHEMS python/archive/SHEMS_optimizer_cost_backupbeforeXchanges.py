...
def SHEMS_optimizer(sh, hp, fh, hw, b, m, pv):
    ...
    # Define variables
    X = pl.LpVariable.dicts("X", ((i, j) for i in range(1, m.h_predict + 1) for j in range(1, len(flows) + 1)), lowBound=0)
    SOC_b = pl.LpVariable.dicts("SOC_b", (i for i in range(1, m.h_predict + 2)), lowBound=0)
    T_fh = pl.LpVariable.dicts("T_fh", (i for i in range(1, m.h_predict + 2)), lowBound=0)
    V_hw = pl.LpVariable.dicts("V_hw", (i for i in range(1, m.h_predict + 2)), lowBound=0)
    Mod_fh = pl.LpVariable.dicts("Mod_fh", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    Mod_hw = pl.LpVariable.dicts("Mod_hw", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    T_fh_plus = pl.LpVariable.dicts("T_fh_plus", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    T_fh_minus = pl.LpVariable.dicts("T_fh_minus", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    V_hw_plus = pl.LpVariable.dicts("V_hw_plus", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    V_hw_minus = pl.LpVariable.dicts("V_hw_minus", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    HP_switch = pl.LpVariable.dicts("HP_switch", (i for i in range(1, m.h_predict + 1)), cat='Binary')
    Hot = pl.LpVariable.dicts("Hot", (i for i in range(1, m.h_predict + 1)), cat='Binary')
    ...
    for key, value in list(X.items())[:5]:
        print(f'{key}: {value}')
    print("printing X(1,3 5 and 7): ", X[(1,1)], X[(1,3)], X[(1,5)], X[(1,7)])#[3], X[1][5], X[1][7], "finished")
    ...
    # Collect returns
    profits = sum((sh.p_sell * pl.value(X[h][5])) - sh.p_buy * sum(pl.value(X[h][i]) for i in [3, 7]) for h in range(1, m.h_control + 1))
    results = [[pl.value(T_fh[h]), pl.value(V_hw[h]), pl.value(SOC_b[h]), pl.value(V_hw_plus[h]), pl.value(V_hw_minus[h]),
                pl.value(T_fh_plus[h]), pl.value(T_fh_minus[h]), profits, cop_fh[h - 1], cop_hw[h - 1], [pl.value(X[h][i]) for i in range(1, len(flows) + 1)],
