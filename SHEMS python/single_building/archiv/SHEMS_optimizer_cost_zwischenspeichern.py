...
def SHEMS_optimizer(sh, hp, fh, hw, b, m, pv):
    ...
    # Define variables
    X = pl.LpVariable.dicts("X", ((i, j) for i in range(0, m.h_predict) for j in range(1, len(flows) + 1)), lowBound=0)
    SOC_b = pl.LpVariable.dicts("SOC_b", (i for i in range(0, m.h_predict + 1)), lowBound=0)
    T_fh = pl.LpVariable.dicts("T_fh", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    V_hw = pl.LpVariable.dicts("V_hw", (i for i in range(1, m.h_predict + 1)), lowBound=0)
    Mod_fh = pl.LpVariable.dicts("Mod_fh", (i for i in range(1, m.h_predict)), lowBound=0)
    Mod_hw = pl.LpVariable.dicts("Mod_hw", (i for i in range(1, m.h_predict)), lowBound=0)
