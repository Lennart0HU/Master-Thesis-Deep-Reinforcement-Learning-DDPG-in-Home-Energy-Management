...
ShemsAction() = ShemsAction(1f0) # ShemsAction(0.7f0, 1f0)

Base.size(::ShemsAction) = (1,) # (2,)
...
function step!(env::Shems, s, a; track=0)
	...
	if track >= 0
		...
		EV = action(env, ShemsAction(EV_target))
		#env.a = ShemsAction(B_target, EV_target) 
		env.a = ShemsAction(EV_target) 
	elseif track < 0
		# B_target, EV_target = zeros(Float32,2)
		EV_target = zeros(Float32,1)
		EV = a
		env.a = ShemsAction(EV_target)
	end

  	pv_, BD, BC, EVC, abort, discomfort, penalty, cost = zeros(8)
	PV_DE, PV_B, PV_EV, PV_GR, B_DE, B_EV, B_GR, GR_DE, GR_EV, GR_B, EX_EV = zeros(11)

	############# DETERMINE FLOWS ###################################
	# BD and BC potential
	BD_p = min(b.rate_max,  ((1 - b.loss) * Soc_b))
	BC_p = min(b.rate_max, b.soc_max - Soc_b)
	...
	if (g_e * pv.eta) > d_e
		...
		if  pv_ > EV
			PV_EV = EV #EV_target reached
			pv_ -= PV_EV
		elseif pv_ <= EV
			PV_EV = pv_
			pv_ = 0
			if BD_p > (EV - PV_EV) / b.eta # EV from battery?
				B_EV = (EV - PV_EV)
				BD_p -= B_EV / b.eta
			elseif BD_p <= (EV - PV_EV) / b.eta
				B_EV = BD_p * b.eta
				BD_p = 0
	...
	elseif (g_e * pv.eta) <= d_e # electr. demand
		...
		pv_ = 0
		d_e -= PV_DE
		if BD_p > (d_e / b.eta) # from battery?
			B_DE = d_e
			BD_p -= B_DE / b.eta
			if BD_p > (EV / b.eta)
				B_EV = EV
				BD_p -= B_EV / b.eta
			elseif BD_p <= (EV / b.eta)
				B_EV = BD_p * b.eta
				BD_p = 0
				GR_EV = EV - B_EV		# slack variable EV
			end
		elseif BD_p <= (d_e / b.eta)
			B_DE = BD_p * b.eta
			BD_p = 0
	...
	elseif pv_ <= (BC_p / b.eta)
		...
		# EV leftover potential
	EV_p = min(ev.rate_max, (1 - Soc_ev) * (ev.soc_max - ev.soc_min) )
	EV_p -= (PV_EV + B_EV)
