...
second_digit = parse(Int, ternary_digits[2])
...
function step!(env::Shems, s, a; track=0)
	...
	if (env.state.Soc_ev - Soc_ev) * ev.soc_max > 11
		print("Alert! old Soc_ev: $(Soc_ev), thats $(Soc_ev * ev.soc_max), new Soc_ev: $(env.state.Soc_ev), thats $(env.state.Soc_ev * ev.soc_max)")
