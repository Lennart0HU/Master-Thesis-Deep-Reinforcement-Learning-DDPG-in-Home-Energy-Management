...
ShemsAction() = ShemsAction(0.7f0, 0.7f0)   #LU 0.7f0, 0.7f0, 0.7f0)

Base.size(::ShemsAction) =  (2,) #LU (3,)
Base.minimum(::ShemsAction) = (0f0, 0f0) #LU (0f0, 0f0, 0f0)
Base.maximum(::ShemsAction) = (1f0, 1f0) #LU (1f0, 1f0, 1f0)
...
function Base.setindex!(a::ShemsAction, x, i::Int)
  (i > length(a)) && throw(BoundsError(a, i))
  setproperty!(a, :B, x) # ifelse(i == 1, :B) #LU ,
	#LU ifelse(i == 2, :FH,
  	#LU :HW)), x)
end
...
#=		if T_fh_perc < FH_target && V_hw_perc < HW_target # under threshold FH/HW %
		...
		if pv_ > 0 && Soc_b_perc < B_target.B #LU B_target
			# Fill up to target state
