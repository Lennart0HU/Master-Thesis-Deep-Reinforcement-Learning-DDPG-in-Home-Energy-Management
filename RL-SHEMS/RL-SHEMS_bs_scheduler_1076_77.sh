...
do
    ...
    if [[ $JOB_ID -eq 10769809 || $JOB_ID -eq 10769827 ]]; then
        JOB_ID=$((JOB_ID + 1))
        continue
    fi
...
while ((JOB_ID <= 10779880))
do

    if [[ $JOB_ID -eq 10779809 || $JOB_ID -eq 10779827 ]]; then
        JOB_ID=$((JOB_ID + 1))
        continue
    fi

    export JOB_ID
    cp input07_GS_test.jl out/input/$JOB_ID--input.jl
