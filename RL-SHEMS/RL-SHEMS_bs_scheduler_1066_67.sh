...
while ((JOB_ID <= 10669826))
do

    if [[ $JOB_ID -eq 10669806 || $JOB_ID -eq 10669818 ]]; then
        JOB_ID=$((JOB_ID + 1))
        continue
    fi

    export JOB_ID
    cp input06.jl out/input/$JOB_ID--input.jl
...
while ((JOB_ID <= 10679826))
do

    if [[ $JOB_ID -eq 10679806 || $JOB_ID -eq 10679818 ]]; then
        JOB_ID=$((JOB_ID + 1))
        continue
    fi

    export JOB_ID
    cp input07.jl out/input/$JOB_ID--input.jl
