...
run = "eval" # "test", "eval"
WAIT_SEC = 600 # 60 * (1 + (parse(Int, Job_ID) % 10) * (parse(Int, Job_ID) % 10))
...
function set_hyperparameters(job_id)
  ...
  last_three_digits = parse(Int, job_id[end-2:end])
  ...
  alt_values = Dict(
      ...
      6 => (0.2f0, 0.1f0),            # σ (omega) alternatives
      7 => (0.15f0, 0.2f0)             # θ (theta) alternatives
  )
  ...
  params = Dict(
      ...
      :γ => alt_values[2][1],
      :τ => alt_values[3][1],
      :η_act => alt_values[4][1],
      :η_crit => alt_values[5][1],
      :σ => alt_values[6][1],
      :θ => alt_values[7][1],
  ...
  binary_digits = string(last_three_digits, base=2, pad=7)  # binary for 2 settings of 7 parameters, so 2^7=128 Job_IDs
  ...
  for i in 1:length(binary_digits)
      ...
      if digit == 1
          if i == 1
            ...
            params[Symbol("γ", "τ", "η_act", "η_crit", "σ", "θ")[i-1]] = alt_values[i][2]
          end
      end
  end
