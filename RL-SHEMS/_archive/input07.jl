...
function set_hyperparameters(job_id)
  ...
  alt_values = Dict(
      1 => (24_000, 20_000, 30_000),
      2 => (0.1, 0, 1),             # Penalty
      3 => (("gn", 0.1, 0.15), ("ou", 0.1, 0.15), ("ou", 0.2, 0.2)) # noise type, omega, theta
  )
  ...
  params = Dict(
      ...
      :σ => alt_values[3][1][2],
      :θ => alt_values[3][1][3],
      :DISCOMFORT_WEIGHT_EV => 1, # REMEMBER TO ADJUST THIS IN ENV
      :penalty => alt_values[2][1], # REMEMBER TO ADJUST THIS IN ENV
      :TRAIN_EP_LENGTH => 72,
      :NUM_EP => 1_001,
      :BATCH_SIZE => 120,
      :MEM_SIZE => alt_values[1][1],
      :noise_type => alt_values[3][1][1]
  )
  ...
  for i in 1:length(ternary_digits)
      ...
      if i == 3
          params[:noise_type], params[:σ], params[:θ] = alt_values[i][digit + 1]  # Unpack the tuple for L1, L2, γ, σ, θ
      else
          params[[:MEM_SIZE, :penalty][i]] = alt_values[i][digit + 1]
      end
  end
  return params
end
