...
test_on_charger = ((parse(Int, Job_ID) ÷ 10000) % 10)
...
run = "test_charger0$test_on_charger"  # "test", "eval"
num_seeds = 40  # always make sure this matches the highest Task_ID in the bash scheduler!
WAIT_SEC = 90 # 60 * (1 + (parse(Int, Job_ID) % 10) * (parse(Int, Job_ID) % 10))
...
EP_LENGTH = Dict("train" => TRAIN_EP_LENGTH,
          ...
          #("all", "eval") => 100,   ("all", "test") => 200) # length of whole evaluation set (different)
					...
					("all", "test_charger0$test_on_charger") => 1439,   ("all", "test_charger0$test_on_charger") => 2999)
...
env_dict = Dict("train" => Shems(EP_LENGTH["train"], "data/$(Charger_ID)_$(season)_train_$(price).csv"),
				...
				"test_charger0$test_on_charger" => Shems(EP_LENGTH[season, "test"], "data/Charger0$(test_on_charger)_$(season)_test_$(price).csv"))
