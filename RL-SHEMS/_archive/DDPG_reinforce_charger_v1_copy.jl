...
ternary_digits = string(last_two_digits, base=3, pad=4)  # ternary for 3 settings of 3 parameter-settings, so 3^3 - 3 = 24 Job_IDs (03:26) (skipping first three that already exist)
fourth_digit = parse(Int, ternary_digits[4])

if fourth_digit == 1
    DISCOMFORT_WEIGHT_EV = 0.01f0
elseif fourth_digit == 2
	DISCOMFORT_WEIGHT_EV = 0.04f0
else
    DISCOMFORT_WEIGHT_EV = 0.1f0
end

println(Job_ID)
println(last_two_digits)
println(ternary_digits)
println(fourth_digit)
println(DISCOMFORT_WEIGHT_EV)
