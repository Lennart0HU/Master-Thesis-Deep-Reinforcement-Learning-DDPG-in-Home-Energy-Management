...
Test_Charger_ID = Charger_ID

#Charger_ID = "Charger98" # ID of the trained model
#Test_Charger_ID = "Charger" * lpad((parse(Int, Job_ID) ÷ 100) % 100, 2, '0') # ID of test data
...
run = "test" # "test", "eval"
WAIT_SEC = 900 # 60 * (1 + (parse(Int, Job_ID) % 10) * (parse(Int, Job_ID) % 10))
...
env_dict = Dict("train" => Shems(EP_LENGTH["train"], "data/$(Charger_ID)_$(season)_train_$(price).csv"),
				...
				"test" => Shems(EP_LENGTH[season, "test"], "data/$(Test_Charger_ID)_$(season)_test_$(price).csv"))
